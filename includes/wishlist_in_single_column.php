...
function d2_add_n_remove_wishlist_userID(){
    ...
    if (isset($_POST['prod_id'])) {
        ...
        $prod_id        = $_POST['prod_id'];
        $prod_ids       = array( $prod_id );
        $product_id     = serialize($prod_ids);
        $current_time   = current_time( 'mysql' );

        $chk_exist_prod = $wpdb->get_row( "SELECT * FROM ".$wpdb->prefix."custom_whishlist WHERE user_id =".$user_id, ARRAY_A );
        
        // Check exist value
        if (empty($chk_exist_prod)) {
            ...
            $check_pro_id = unserialize( $chk_exist_prod['product_id'] );
            // If Exist Product Key
            if(in_array( $prod_id, $check_pro_id )){

                echo json_encode(array('type' => 'already' , 'msg' => 'Already Add on Wishlist' ));
            
            }else{
             
                $exist_table_data   = unserialize( $chk_exist_prod['product_id'] );
                $merge_data         = array_merge( $exist_table_data , $prod_ids );
                $new_product_id     = serialize( $merge_data );

                $results = $wpdb->update( 
                    $wpdb->prefix.'custom_whishlist', 
                    array( 
                        'product_id' => $new_product_id,  // string
                    ), 
    ...
    if (isset($_POST['del_prod_id']) && $_POST['del_prod_id']) {

        $del_user_id = $_POST['del_user_id'];
        $del_prod_id = $_POST['del_prod_id'];

        $del_exist_prod = $wpdb->get_row( "SELECT * FROM ".$wpdb->prefix."custom_whishlist WHERE user_id =".$del_user_id, ARRAY_A );

        $get_all_data = unserialize( $del_exist_prod['product_id'] );
        // Search
        $delt_prod_id = array_search($del_prod_id, $abc);
        // Remove from array
        if ($delt_prod_id) {
            //$unset_data = unset ( $get_all_data[ $delt_prod_id ] );
            //unset($dev['2']);
        }
        
        $del_data = serialize($unset_data);
        
        // update Query
        $results = $wpdb->update( $wpdb->prefix.'custom_whishlist', 
            array( 'product_id' => $del_data ),
            array( 'user_id' => $del_user_id ),
            array( '%s', '%d' ),
            array( '%d' ) 
        );
        // On success
...
function d2_wishlist_table( $atts, $content = null ) {
    ...
    if (is_user_logged_in()) {
        ...
        $user_prod_wishlist = $wpdb->get_row( "SELECT * FROM ".$wpdb->prefix."custom_whishlist WHERE user_id =".$user_id, ARRAY_A );
        if ($user_prod_wishlist) {
            
            $product_id = unserialize($user_prod_wishlist['product_id']);

            if ($product_id) {
                foreach ($product_id as $key => $value) {
                    $wishlist_product_id[].= $value;
                }
            }
        }
